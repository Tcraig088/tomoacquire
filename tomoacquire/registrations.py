...
class MicroscopeItemDict():
    ...
    def __new__(cls):
        if cls._instance is None:
            cls._instance = super(MicroscopeItemDict, cls).__new__(cls)
    ...
    def update(self):
        ...
        tiltscheme_path = os.path.join(path, 'microscopes')
        attribute_name = 'is_tomoacquire_microscope'
    ...
    def append(self, key, value):
        ...
        if key in self._dict:
            ...
            if isinstance(value, type) and issubclass(value, QWidget):
                self._dict[key] = MicroscopeTypeItem(self._items, widget=value)
            else:
                self._dict[key] = MicroscopeTypeItem(self._items, controller=value)
            self._items += 1
