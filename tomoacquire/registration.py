...
class TiltSchemeItemDict():
    ...
    def update(self):
        ...
        tomoacquire_path = os.path.dirname(spec.origin)
        folder_path = os.path.join(tomoacquire_path, 'plugins', 'tiltschemes')
        attribute_name = 'is_tomoacquire_tiltscheme'
        
        logger.info(f"Updating TiltSchemeItemDict from {folder_path}")
